...
2. Build and publish on the server. Plesk Git auto deploys main to app_src but only copies source; it does not build the web or restart the Brain unless the build script is saved as the Plesk Additional deployment action. The reliable path is to run that same build as the site user over SSH:
   `sudo -u legenex.com_vvdx4ff41il bash -lc 'set -e; APP=/var/www/vhosts/legenex.com/nexa.legenex.com/app_src; cd "$APP"; git pull --ff-only; pnpm install --frozen-lockfile=false; pnpm --filter web build; rsync -a --delete apps/web/dist/ ../httpdocs/; cd services/brain; [ -d .venv ] || python3 -m venv .venv; .venv/bin/pip install -q --upgrade pip; .venv/bin/pip install -q .; sudo systemctl restart nexa-brain; echo DEPLOY_OK'`
   The leading `git pull --ff-only` makes the SSH build self-contained: it pulls the newest commits before building, so it works whether or not Plesk has already copied the source. To set the owner or admin password directly on the live Postgres DB (for example a lockout recovery), load the secrets first since the DB URL lives in brain.env, not services/brain/.env: `set -a; . /var/www/vhosts/legenex.com/nexa.legenex.com/brain.env; set +a; .venv/bin/python -m scripts.create_user nick@legenex.com 'NEWPASS' --role owner` (run from services/brain). Without loading brain.env the script falls back to an empty local SQLite file and fails with "no such table: users".
