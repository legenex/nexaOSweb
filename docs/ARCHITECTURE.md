...
### Autonomy dial and risk gating (AB4.3)
Every task carries an autonomy level, a dial with three settings. Green runs start to finish unattended (the build run auto resolves its own Human Gate and merges). Yellow pauses at the Human Gate for approval. Red never auto runs and the specific destructive action is refused at the command layer. On top of the dial sits a deterministic classifier (app/autonomy.py), not a learned model and not the agent's judgement: it marks any change touching auth, payments or money, database migrations, deploy or infra, or secrets as at least yellow, and the destructive or irreversible actions (file deletion, a git force push, a push to a protected branch, dropping or truncating data, a hard reset or history rewrite) as red, regardless of the level a task was set to. The classifier only ever escalates the risk (lowers autonomy), never relaxes it. A project carries an autonomy default new tasks inherit and a kill switch that halts in flight runs and refuses new ones. The agent proposes; this layer disposes.

### Orchestrator loop (AB4.2)
The orchestrator (app/agents/orchestrator.py) walks a sliced, approved project's task graph in dependency order and dispatches a build run for each ready task through start_build_run on the in-process worker, respecting per task autonomy through the same gate_decision the single run path uses. Green tasks auto advance and the loop continues to their dependents; yellow and red tasks park at the gate and the loop pauses that branch until a human resolves it. The orchestrator never bypasses classify_risk and never force merges. The project kill switch halts the whole loop immediately and refuses new dispatch until released. A run cap and a wall-clock budget per project bound the loop; on hitting either it pauses and reports. Every dispatch, gate decision, and pause is recorded on the project's PMRun.state and on each run.plan, so the loop is fully auditable. The approval gate lives here, at orchestrate, not at slice time: a plan may be sliced into tasks for inspection before the project is approved, but the loop refuses to run an unapproved project.

The orchestrator loop is gated behind the NEXA_ENABLE_ORCHESTRATOR flag, which defaults false. While it is off the orchestrate endpoint is refused, so the loop cannot dispatch a real agent. The flag stays off until both of these are true:
1. The AB2.1 live single-run acceptance has passed against the real Claude Code CLI: a real benign diff captured, approved through the executor merge, rejected through rollback, a green task auto-merged on its own, and an auth-file edit forced to the gate. (To date this has been proven only with a stub CLI in tests, never against the real CLI.)
2. The per-task autonomy projection is in place so the UI cannot show a task as more gated than it is. Done in AB4.2: TaskRead and the task graph both project the stored per-task autonomy level, and the web AutonomySelector reads it on reopen rather than falling back to the project default.

The flag is not removed; turning it on after both preconditions hold is a deliberate, separate go-live step.

### Governance audit log and cost accounting (AB5.2, AB5.3)
Every governed moment of the build engine writes one immutable row to an append-only audit log (the agent_audit table, written only through app/audit.py). The events are the run start, the backend selection trail, the gate decision (effective level, categories, reasons), every approve, reject, and cancel, every kill switch engage and release, and every orchestrator pause and resume. Each row names an actor (a user or the system), a reason, and a timestamp, and is scoped to a project and run so the reads (GET /agents/audit, GET /agents/projects/{id}/audit) can filter and owner-scope. The table is append-only by construction: two ORM mapper listeners refuse any update or delete, so an event can never be altered or hard deleted, only added to. Secrets never reach a row; the writer runs the same redaction guard the backends apply to their transcripts before insert. Retention lives in AppSetting (default keep all), so the window tunes without a schema change. The producers are wired at the real call sites in build_engine and orchestrator, not at the read layer, so the log reflects what actually happened.

Cost is accounted from the real usage the backends report and the engine records: cost_usd is denormalised on each build run and the token counts live on each run's build step evidence, so no usage is duplicated into a new column. GET /agents/projects/{id}/cost rolls a project's spend and token totals up by backend from that stored truth. Two enforcement paths consume it. The per backend cost ceiling in config/agents.yaml is enforced by feeding the selector a per backend estimate (the mean of the project's prior runs on that backend); a candidate whose projected cost is over its ceiling is skipped with a recorded reason, captured in the backend selection audit trail. A daily and monthly project budget lives in AppSetting (default unlimited); when a project is over budget the orchestrator pauses dispatch by reusing the pause path, which records the audit pause event.

### Deferred outcome seam (AB5.3, recording only)
The OutcomeLog table (the outcome_log table, written only through app/outcomes.py) records, per completed build run, the human verdict (approved or rejected), whether a merged change was later reverted, and a free text note. It is populated from the existing approve, reject, and revert paths in the build engine: approve records the approved verdict, a plain reject records rejected, and a reject of a run whose change had already merged keeps the approved verdict and sets reverted. There is at most one row per run; the writer upserts and a revert is sticky. This is the recording seam only. No ranking, scoring, or learning is built on these rows now; turning the recorded outcomes into a signal that shapes backend selection or autonomy is a deferred milestone, and capturing them durably from day one is what makes that possible later without a backfill.
...
DATABASE_URL, NEXA_SESSION_SECRET, NEXA_PUBLIC_HTTPS, NEXA_DESKTOP_BEARER, NEXA_PROJECTS_ROOT, NEXA_UPLOADS_ROOT, NEXA_RUNTIME_ROOT (where large runtime tool output is stored by reference), CORS_ORIGINS, and the provider keys ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, TAVILY_API_KEY, XAI_API_KEY (for the optional Grok Build backend). Provider keys are read only by the Brain. Feature flags: NEXA_ENABLE_GROK (the Grok Build backend, default false) and NEXA_ENABLE_ORCHESTRATOR (the unattended orchestrator loop, default false, kept off until the go-live preconditions under Agent Build Engine hold). The orchestrator bounds NEXA_ORCHESTRATOR_RUN_CAP (max dispatches per loop) and NEXA_ORCHESTRATOR_BUDGET_SECONDS (wall-clock budget per loop) keep a loop from running unbounded.
