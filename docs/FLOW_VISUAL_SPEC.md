...
- Holographic page object (HoloObject). A reusable component that renders a small cyber holographic wireframe in SVG from the brand variables, slowly rotating and floating with a subtle pointer parallax. It is aria hidden and pointer-events none, sized to sit in a page header or empty corner behind the content layer so it never blocks content. A variant prop gives each built out surface a distinct form: dashboard a cube, insights crossed orbital rings, research a faceted diamond, project-builder a stacked ziggurat, projects a geodesic hex ball. It reuses the panorama's SVG approach and adds no dependency. Under prefers reduced motion it renders a still frame with no rotation, float, or parallax.
