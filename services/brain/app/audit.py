...
The build engine never inserts an AgentAudit row directly. It calls record_audit, or one of the
thin event helpers below, so every audit row is validated, scoped, and redaction checked the same
way. The log is append-only at the ORM (see app/models/audit.py); this module is the only place
that adds to it.

Three invariants hold for every row:
  1. The (category, action) pair is one this module knows. An unknown pair is a programming
     error, raised, never silently logged.
  2. project_id is backfilled from the run when the caller passes only a run, so each row is
     self describing and the reads can scope by project with one column.
  3. No secret reaches a row. The reason and the structured detail are walked by the redaction
     guard (the same field name guard the adapters apply) before insert.

Retention lives in AppSetting, not in a column, so it can change without a migration. The default
is keep all: nothing is ever pruned until an operator configures a window.
"""
...
def record_audit(
    ...
    *,
    ...
    # it can be persisted. The same guard the adapters apply, reused here so secrets stay server
